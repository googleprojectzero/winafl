/*
   ...
   CUDA GPU acceleration contributed by Elias Ibrahim <elie.ibrahim@gmail.com> (Feb 2026)
...
#include <errno.h>
#ifdef HAVE_CUDA
/* CUDA GPU Acceleration support contributed by Elias Ibrahim <elie.ibrahim@gmail.com> (Feb 2026) */
#include "afl-fuzz-gpu.h"
#endif

#ifdef USE_GPU_SANDBOX
#include "winafl_integration.h"
#endif
...
#ifdef HAVE_CUDA
u8 use_gpu = 0;
// GPUBatch gpu_batch - Removed, using singleton
u32 gpu_batch_size = 10000;
u64 gpu_batches_submitted = 0;
u64 gpu_total_execs = 0;        /* Total mutations generated by GPU */
#endif

#ifdef USE_GPU_SANDBOX
u8 use_gpu_sandbox = 0;
#endif
...
#ifdef HAVE_CUDA
  if (use_gpu) {
    SAYF(bSTG bV bSTOP "  gpu status  : " cLGN "ONLINE" cRST
         "  batches=%-6s execs=%-10s " bSTG bV "\n",
         DI(gpu_batches_submitted), DI(gpu_total_execs));
  } else {
    SAYF(bSTG bV bSTOP "  gpu status  : " cLRD "OFFLINE" cRST
         " (add -G flag to enable GPU mutations)         " bSTG bV "\n");
  }
#endif
...
havoc_stage:
  ...
  /* GPU havoc block removed: was using stage_max before it was set */
  ...
  stage_cur = 0;
#ifdef HAVE_CUDA
  if (use_gpu && gpu_is_available() && stage_max > 200) {
      u32 gpu_ops = stage_max;
      size_t total_computed = 0;
      int current_buf = 0;
      u32 batch_sz = (gpu_ops > gpu_batch_size) ? gpu_batch_size : gpu_ops;

      // Kick off first batch on buffer 0
      if (gpu_mutate_batch_async(current_buf, out_buf, len, batch_sz) == 0) {

          while (total_computed < gpu_ops) {
               u32 remaining = gpu_ops - (u32)total_computed;
               u32 current_batch = (remaining > gpu_batch_size) ? gpu_batch_size : remaining;

               u32 next_remaining = remaining - current_batch;
               u32 next_batch = (next_remaining > gpu_batch_size) ? gpu_batch_size : next_remaining;

               int next_buf = 1 - current_buf;
               if (next_batch > 0) {
                   gpu_mutate_batch_async(next_buf, out_buf, len, next_batch);
               }

               // Wait for current buffer's GPU->Host transfer
               gpu_sync(current_buf);

               for (u32 k = 0; k < current_batch; k++) {
                   size_t m_len = gpu_get_mutation(current_buf, k, out_buf, MAX_FILE);
                   if (m_len > 0) {
                       if (common_fuzz_stuff(argv, out_buf, m_len)) goto abandon_entry;
                   }
                   stage_cur++;
               }

               total_computed += current_batch;
               gpu_batches_submitted++;
               gpu_total_execs += current_batch;
...
int main(int argc, char** argv) {
  ...
  /* Rewrite -gpu or /gpu to -G to support the requested flag style. */
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:I:T:sdyYnCB:S:M:x:QD:b:l:pPc:w:A:eVGX")) > 0)
    ...
    switch (opt) {
      ...
      case 'G':
        ...
        break;
#else
        FATAL("System not configured for GPU acceleration");
#endif
      ...
      case 'X':
        ...
        break;
#else
        FATAL("System not configured for GPU Sandbox");
#endif
...
#ifdef USE_GPU_SANDBOX
  if (!in_dir || !out_dir || !timeout_given || (!drioless && !dynamorio_dir && !use_intelpt && !use_tinyinst && !use_gpu_sandbox)) usage(argv[0]);
#else
...
#ifdef HAVE_CUDA
  if (use_gpu) {
      char* gpu_batch_env = getenv("AFL_GPU_BATCH_SIZE");
      if (gpu_batch_env) {
          gpu_batch_size = atoi(gpu_batch_env);
          if (gpu_batch_size < 1) gpu_batch_size = 10000;
      }
      ACTF("Initializing GPU acceleration (Batch Size: %u)...", gpu_batch_size);
      ACTF("GPU support added by Elias Ibrahim <elie.ibrahim@gmail.com>");
      if (gpu_init() != 0) {
          FATAL("GPU initialization failed");
      }
      OKF("GPU initialized successfully");
  }
#endif
  ...
  /* GPU init moved to guarded block above (only when -G is specified) */
...
#ifdef USE_GPU_SANDBOX
  if (use_gpu_sandbox) {
      ACTF("Entering GPU Sandbox mode...");
      sandbox_fuzz_loop(in_dir, out_dir, NULL);
  } else {
#endif
  while (1) {
