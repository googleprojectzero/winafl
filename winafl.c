...
createfilew_interceptor(void *wrapcxt, INOUT void **user_data)
{
	...
	if(wcsstr(filenamew, L"___FILE___")) {
		drwrap_set_arg(wrapcxt, 0, fuzz_target.testcase_filename_w);
	}
...
createfilea_interceptor(void *wrapcxt, INOUT void **user_data)
{
	...
	if(strstr(filename, "___FILE___")) {
		drwrap_set_arg(wrapcxt, 0, fuzz_target.testcase_filename);
	}
...
setup_pipe() {
	...
	//LPTSTR pipe_name = TEXT("\\\\.\\pipe\\winaflpipe"); 
