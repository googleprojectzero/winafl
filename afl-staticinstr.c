...

{
    ...
    for(UINT32 i = 0; i < SizeNeeded / sizeof(Modules[0]); ++i) {
        ...
        for(USHORT j = 0; j < NtHeaders->FileHeader.NumberOfSections; ++j) {
    ...
    if(_stscanf_s(StaticConfig, TEXT("%[a-zA-Z0-9]:%zu"), FuzzerId, _countof(FuzzerId), &g_niterations) != 2) {
        _tprintf(
            TEXT("[-] The " AFL_STATIC_CONFIG_ENV " environment variable isn't properly formated.\n")
        );
        Status = FALSE;
        goto clean;
    }
    ...
    for(SIZE_T i = 0; i < g_ninstrumented_modules; ++i) {
...

{
...

    {
        ...
        if(g_current_iterations == 1) {
            ...
            for(SIZE_T i = 1; i < g_ninstrumented_modules; ++i) {
